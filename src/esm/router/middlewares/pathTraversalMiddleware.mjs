// src/mjs/router/middlewares/pathTraversalMiddleware.mjs

import express from "express";
import path from "node:path";
import { getClientIp } from "../../utils.mjs";
import { httpBadRequest, httpForbidden } from "../exceptionAPI.mjs";
 ...
 *
 * Por padrão, utiliza o diretório de trabalho
 * em que o processo Node.js foi iniciado.
 *
 * @param {Object} [options] Configurações do middleware.
 * @param {string} [options.query="file"] Parâmetro da query que contém o caminho.
...
function createPathTraversalMiddleware({
	...
	return function pathTraversalMiddleware(req, res, next) {
		...
		if (!safePath) {
			...
			return httpForbidden(res,"\n\t[npm-package-nodejs-utils-lda] [pathTraversal]","Acesso negado.")
		}

		/*
		 * Disponibiliza o caminho validado para a aplicação.
		 */
 ...
 *
 * Por padrão, o diretório protegido será o diretório
 * de trabalho atual do processo Node.js.
 *
